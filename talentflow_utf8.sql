...
ALTER TABLE ONLY public.assessment_responses
    ADD CONSTRAINT assessment_responses_candidate_id_fkey FOREIGN KEY (candidate_id) REFERENCES auth.users(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.assessment_sessions
    ADD CONSTRAINT assessment_sessions_candidate_id_fkey FOREIGN KEY (candidate_id) REFERENCES auth.users(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.candidate_job_sync
    ADD CONSTRAINT candidate_job_sync_candidate_id_fkey FOREIGN KEY (candidate_id) REFERENCES auth.users(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.candidate_settings
    ADD CONSTRAINT candidate_settings_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.profile_analytics
    ADD CONSTRAINT profile_analytics_candidate_id_fkey FOREIGN KEY (candidate_id) REFERENCES auth.users(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.profile_analytics
    ADD CONSTRAINT profile_analytics_recruiter_id_fkey FOREIGN KEY (recruiter_id) REFERENCES auth.users(id) ON DELETE SET NULL;
...
CREATE POLICY "Allow users to check their own blocked status" ON public.blocked_users FOR SELECT TO authenticated USING ((auth.uid() = user_id));
...
CREATE POLICY "Authenticated users can view company info" ON public.companies FOR SELECT USING ((auth.uid() IS NOT NULL));
...
CREATE POLICY "Candidate can read own profile" ON public.candidate_profiles FOR SELECT USING ((user_id = auth.uid()));
...
CREATE POLICY "Candidates can apply for jobs" ON public.job_applications FOR INSERT WITH CHECK ((auth.uid() = candidate_id));
...
CREATE POLICY "Candidates can manage own applications" ON public.job_applications USING ((candidate_id = auth.uid()));
...
CREATE POLICY "Candidates can read own analytics" ON public.profile_analytics FOR SELECT USING ((candidate_id = auth.uid()));
...
CREATE POLICY "Candidates can select slots" ON public.interview_slots FOR UPDATE USING ((EXISTS ( SELECT 1
  ...
  WHERE ((interviews.id = interview_slots.interview_id) AND (interviews.candidate_id = auth.uid())))));
...
CREATE POLICY "Candidates can update interview slot selection" ON public.interviews FOR UPDATE USING ((auth.uid() = candidate_id));
...
CREATE POLICY "Candidates can view own analytics" ON public.profile_analytics FOR SELECT USING ((candidate_id = auth.uid()));
...
CREATE POLICY "Candidates can view own sync insights" ON public.candidate_job_sync FOR SELECT USING ((candidate_id = auth.uid()));
...
CREATE POLICY "Candidates can view their interviews" ON public.interviews FOR SELECT USING ((auth.uid() = candidate_id));
...
CREATE POLICY "Recruiter can read own profile" ON public.recruiter_profiles FOR SELECT USING ((user_id = auth.uid()));
...
CREATE POLICY "Recruiters can insert analytics" ON public.profile_analytics FOR INSERT WITH CHECK ((auth.uid() IS NOT NULL));
...
CREATE POLICY "Recruiters can manage slots" ON public.interview_slots USING ((EXISTS ( SELECT 1
   FROM public.interviews
  WHERE ((interviews.id = interview_slots.interview_id) AND (interviews.recruiter_id = auth.uid())))));
...
CREATE POLICY "Recruiters can manage their own jobs" ON public.jobs USING ((auth.uid() = recruiter_id));
...
CREATE POLICY "Recruiters can update their interviews" ON public.interviews FOR UPDATE USING (((auth.uid() = recruiter_id) OR (EXISTS ( SELECT 1
   FROM public.jobs
  WHERE ((jobs.id = interviews.job_id) AND (jobs.recruiter_id = auth.uid()))))));
...
CREATE POLICY "Recruiters can view assessment questions" ON public.recruiter_assessment_questions FOR SELECT USING ((auth.role() = 'authenticated'::text));
...
CREATE POLICY "Recruiters can view completed candidate profiles" ON public.candidate_profiles FOR SELECT TO authenticated USING (((EXISTS ( SELECT 1
   FROM public.recruiter_profiles
  WHERE ((recruiter_profiles.user_id = auth.uid()) AND (recruiter_profiles.account_status = 'Active'::public.account_status)))) AND (assessment_status = 'completed'::public.assessment_status)));
...
CREATE POLICY "Recruiters can view their interviews" ON public.interviews FOR SELECT USING (((auth.uid() = recruiter_id) OR (EXISTS ( SELECT 1
   FROM public.jobs
  WHERE ((jobs.id = interviews.job_id) AND (jobs.recruiter_id = auth.uid()))))));
...
CREATE POLICY "Recruiters view applicant profiles" ON public.candidate_profiles FOR SELECT USING ((EXISTS ( SELECT 1
  ...
  WHERE ((job_applications.candidate_id = candidate_profiles.user_id) AND (jobs.recruiter_id = auth.uid())))));
...
CREATE POLICY "User can read own profile score" ON public.profile_scores FOR SELECT USING ((user_id = auth.uid()));
...
CREATE POLICY "Users can check their own blocked status" ON public.blocked_users FOR SELECT USING ((auth.uid() = user_id));
...
CREATE POLICY "Users can manage own candidate profile" ON public.candidate_profiles USING ((user_id = auth.uid())) WITH CHECK ((user_id = auth.uid()));
...
CREATE POLICY "Users can manage own posts" ON public.posts USING ((user_id = auth.uid()));
...
CREATE POLICY "Users can manage own recruiter profile" ON public.recruiter_profiles USING ((user_id = auth.uid())) WITH CHECK ((user_id = auth.uid()));
...
CREATE POLICY "Users can manage own saved jobs" ON public.saved_jobs USING ((candidate_id = auth.uid()));
...
CREATE POLICY "Users can manage their own responses" ON public.assessment_responses TO authenticated USING ((auth.uid() = candidate_id)) WITH CHECK ((auth.uid() = candidate_id));
...
CREATE POLICY "Users can manage their own resume data" ON public.resume_data USING ((auth.uid() = user_id));
...
CREATE POLICY "Users can manage their own session" ON public.assessment_sessions TO authenticated USING ((auth.uid() = candidate_id)) WITH CHECK ((auth.uid() = candidate_id));
...
CREATE POLICY "Users can pin their own" ON public.user_pinned_posts FOR INSERT WITH CHECK ((auth.uid() = user_id));
...
CREATE POLICY "Users can see who follows them" ON public.follows FOR SELECT USING ((auth.uid() = following_id));
...
CREATE POLICY "Users can unpin their own" ON public.user_pinned_posts FOR DELETE USING ((auth.uid() = user_id));
...
CREATE POLICY "Users can update their own candidate settings" ON public.candidate_settings FOR UPDATE USING ((auth.uid() = user_id));
...
CREATE POLICY "Users can view their own candidate settings" ON public.candidate_settings FOR SELECT USING ((auth.uid() = user_id));
...
CREATE POLICY "Users can view their own pins" ON public.user_pinned_posts FOR SELECT USING ((auth.uid() = user_id));
...
CREATE POLICY "Viewable slots" ON public.interview_slots FOR SELECT USING ((EXISTS ( SELECT 1
   FROM public.interviews
  WHERE ((interviews.id = interview_slots.interview_id) AND ((interviews.candidate_id = auth.uid()) OR (interviews.recruiter_id = auth.uid()))))));
...
CREATE POLICY admin_manage_invites ON public.team_invitations USING ((EXISTS ( SELECT 1
   FROM public.recruiter_profiles
  WHERE ((recruiter_profiles.user_id = auth.uid()) AND (recruiter_profiles.is_admin = true) AND (recruiter_profiles.company_id = team_invitations.company_id)))));
...
CREATE POLICY tf_candidate_gps_access ON public.career_gps USING ((candidate_id = auth.uid()));
...
CREATE POLICY tf_candidate_milestone_access ON public.career_milestones USING ((EXISTS ( SELECT 1
  ...
  WHERE ((career_gps.id = career_milestones.gps_id) AND (career_gps.candidate_id = auth.uid())))));
