...
import { Bell, Sparkles, Search, MessageSquare, Users } from "lucide-react";
...
const pageTitles: Record<string, { title: string; sub: string }> = {
  "/dashboard/recruiter": { title: "Dashboard", sub: "Manage your hiring activity" },
  "/dashboard/recruiter/hiring/jobs": { title: "Jobs", sub: "Manage job postings" },
  "/dashboard/recruiter/hiring/jobs/new": { title: "Post a job", sub: "Create a new job listing" },
  "/dashboard/recruiter/hiring/applications": { title: "Applications", sub: "Review candidate applications" },
  "/dashboard/recruiter/talent-pool": { title: "Talent Pool", sub: "Browse verified candidates" },
  "/dashboard/recruiter/intelligence/recommendations": { title: "Recommendations", sub: "AI-matched candidates" },
...
export default function RecruiterHeader() {
  ...
  const navPages = [
    ...
    { label: "Jobs", href: "/dashboard/recruiter/hiring/jobs" },
    ...
    { label: "Team", href: "/dashboard/recruiter/organization/team" },
    { label: "Messages", href: "/dashboard/recruiter/messages" },
    { label: "Community", href: "/dashboard/recruiter/organization/community" },
    { label: "Settings", href: "/dashboard/recruiter/account/settings" },
  ];
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (prof) setProfile({ full_name: prof.full_name, team_role: prof.team_role, profile_photo_url: prof.profile_photo_url || prof.professional_persona?.profile_photo_url });
      } catch {}
    }
    load();
  ...
  const isJobsActive = pathname.includes("/hiring/jobs") || pathname.includes("/hiring/applications");
  const isTalentActive = pathname.includes("/talent-pool") || pathname.includes("/intelligence/recommendations");
  const isOrgActive = pathname.includes("/organization/team");
    ...
    <header className="fixed top-0 left-0 right-0 z-20 bg-white/80 backdrop-blur-xl border-b border-slate-200/60 transition-all duration-300 w-full">
      <div className="h-16 flex items-center px-6 justify-between gap-4">
        ...
        <div className="flex items-center gap-4 flex-shrink-0">
          <div className="flex items-center gap-3 cursor-pointer select-none" onClick={() => router.push("/dashboard/recruiter")}>
            ...
            <div className="hidden md:flex flex-col">
              ...
              <span className="text-[10px] text-slate-400 font-medium tracking-wide uppercase leading-none mt-0.5">Recruiter</span>
            </div>
          </div>
        ...
        <div className="flex-1 flex justify-center items-center gap-3">
          ...
          <div className="relative group py-2">
            ...
            <div className="absolute left-1/2 -translate-x-1/2 top-full pt-1 w-44 hidden group-hover:block z-50">
              <div className="bg-white border border-slate-200/85 rounded-xl shadow-lg py-1.5 overflow-hidden">
                <Link href="/dashboard/recruiter/hiring/jobs/new" className="block px-4 py-2 text-[13px] text-slate-600 hover:bg-slate-50 hover:text-slate-900 font-medium transition-colors">
                  Post a Job
                </Link>
                <Link href="/dashboard/recruiter/hiring/jobs" className="block px-4 py-2 text-[13px] text-slate-600 hover:bg-slate-50 hover:text-slate-900 font-medium transition-colors">
                  Active Jobs
                </Link>
                <Link href="/dashboard/recruiter/hiring/applications" className="block px-4 py-2 text-[13px] text-slate-600 hover:bg-slate-50 hover:text-slate-900 font-medium transition-colors">
                  Applications
                </Link>
              </div>
            </div>
          ...
          <div className="relative group py-2">
            ...
            <div className="absolute left-1/2 -translate-x-1/2 top-full pt-1 w-44 hidden group-hover:block z-50">
              <div className="bg-white border border-slate-200/85 rounded-xl shadow-lg py-1.5 overflow-hidden">
                <Link href="/dashboard/recruiter/talent-pool" className="block px-4 py-2 text-[13px] text-slate-600 hover:bg-slate-50 hover:text-slate-900 font-medium transition-colors">
                  Talent Pool
                </Link>
                <Link href="/dashboard/recruiter/intelligence/recommendations" className="block px-4 py-2 text-[13px] text-slate-600 hover:bg-slate-50 hover:text-slate-900 font-medium transition-colors">
                  Recommendations
                </Link>
              </div>
            </div>
          ...
          <div className="relative group py-2">
            ...
            <div className="absolute left-1/2 -translate-x-1/2 top-full pt-1 w-44 hidden group-hover:block z-50">
              <div className="bg-white border border-slate-200/85 rounded-xl shadow-lg py-1.5 overflow-hidden">
                <Link href="/dashboard/recruiter/organization/team" className="block px-4 py-2 text-[13px] text-slate-600 hover:bg-slate-50 hover:text-slate-900 font-medium transition-colors">
                  Team
                </Link>
              </div>
            </div>
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          ...
          {/* Community Link */}
          <button onClick={() => router.push("/dashboard/recruiter/organization/community")} className={`relative h-9 w-9 flex items-center justify-center rounded-lg hover:bg-slate-100/80 transition-all cursor-pointer ${pathname.includes("/community") ? "text-[#FF8A00] bg-orange-50/50" : "text-slate-400"}`} title="Community">
              ...
              <div className="absolute right-0 mt-2 w-52 bg-white border border-slate-200 rounded-xl shadow-[0_8px_30px_rgba(0,0,0,0.12)] overflow-hidden z-50">
                {profile && (
                  <div className="px-4 py-2.5 border-b border-slate-100 bg-slate-50/50">
                    <p className="text-[13px] font-semibold text-slate-800 truncate">{profile.full_name || "Recruiter"}</p>
                    <p className="text-[11px] text-slate-400 capitalize truncate">{profile.team_role || "Recruiter"}</p>
                  </div>
                )}
                <div className="p-1">
                  <button onClick={() => { router.push("/dashboard/recruiter"); setIsProfileOpen(false); }} className="w-full text-left px-3 py-1.5 text-slate-600 hover:bg-slate-50 hover:text-slate-900 rounded-lg text-[12.5px] font-medium transition-colors">
                    Dashboard
                  </button>
                  <button onClick={() => { router.push("/dashboard/recruiter/account/profile"); setIsProfileOpen(false); }} className="w-full text-left px-3 py-1.5 text-slate-600 hover:bg-slate-50 hover:text-slate-900 rounded-lg text-[12.5px] font-medium transition-colors">
                    Profile
                  </button>
                  <button onClick={() => { router.push("/dashboard/recruiter/account/settings"); setIsProfileOpen(false); }} className="w-full text-left px-3 py-1.5 text-slate-600 hover:bg-slate-50 hover:text-slate-900 rounded-lg text-[12.5px] font-medium transition-colors">
                    Settings
                  </button>
