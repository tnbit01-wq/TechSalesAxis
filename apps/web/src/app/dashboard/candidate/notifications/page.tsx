...
function NotifIcon({ type }: { type: string }) {
  if (type === "ASSESSMENT_REMINDER") return <AlertCircle className="h-4 w-4" />;
  if (type === "INTERVIEW_PROPOSED") return <Calendar className="h-4 w-4" />;
  if (type === "APPLICATION_UPDATE") return <CheckCircle2 className="h-4 w-4" />;
  ...
  return (
    <div className="flex flex-col h-full px-4 sm:px-6 py-4 gap-3 overflow-hidden">
        ...
        {filteredNotifications.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center gap-3 py-16">
            ...
            <p className="text-xs text-slate-300">
              {filter === "unread" ? "Switch to 'All' to see everything" : "We'll notify you when something happens"}
            </p>
          </div>
        ) : (
                ...
                >
                  ...
                  >
                    {notif.type === "INTERVIEW_PROPOSED" && !notif.is_read && (
                      <button
                        onClick={() => { markAsRead(notif.id); handleAction(notif); }}
                        className="px-2 py-1 rounded-lg bg-amber-500 text-white text-[10px] font-bold hover:bg-amber-600 transition-colors whitespace-nowrap"
                      >
                        Respond
                      </button>
                    )}
                    {notif.type === "ASSESSMENT_REMINDER" && !notif.is_read && (
                      <button
                        onClick={() => { markAsRead(notif.id); handleAction(notif); }}
                        className="px-2 py-1 rounded-lg bg-blue-500 text-white text-[10px] font-bold hover:bg-blue-600 transition-colors whitespace-nowrap flex items-center gap-1"
                      >
      ...
      <CandidateInterviewConfirmModal
        ...
        onSuccess={() => { loadNotifications(); setIsModalOpen(false); }}
      />
    </div>
  );
}
