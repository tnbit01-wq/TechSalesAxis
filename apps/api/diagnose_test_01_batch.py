...
try:
    ...
    if stats and stats['total'] > 0:
        ...
        avg_years = f"{stats['avg_years']:.1f}" if stats['avg_years'] else "N/A"
        print(f"Avg Years: {avg_years}")
    ...
    cursor.execute("""
        SELECT 
            COALESCE(experience::text, 'N/A') as band,
            COUNT(*) as count,
            AVG(years_of_experience) as avg_years
        FROM candidate_profiles cp
        JOIN users u ON cp.user_id = u.id
    ...
    for band in bands:
        avg_years = f"{band['avg_years']:.1f}" if band['avg_years'] else "N/A"
        print(f"  {band['band']:<15} - Count: {band['count']:3d}, Avg Years: {avg_years}")
    ...
    cursor.execute("""
        SELECT 
            ...
            CAST(CAST((buf.parsed_data->>'years_of_experience') AS NUMERIC) AS INTEGER) as extracted_years,
            cp.years_of_experience as profile_years,
            cp.current_role,
            experience::text as experience_band,
            u.email
        ...
        WHERE bu.batch_name = 'test_01'
            ...
            AND buf.parsed_data->>'years_of_experience' IS NOT NULL
    ...
    for row in files_data:
        if row['extracted_years'] != row['profile_years']:
            ...
            print(f"    Extracted: {row['extracted_years']} years  →  Stored: {row['profile_years']} years")
    ...
    for role in roles:
        avg_years = f"{role['avg_years']:.1f}" if role['avg_years'] else "N/A"
        print(f"  {role['role'][:50]:<50} - Count: {role['count']:2d}, Avg Years: {avg_years}")
    ...
    for status in statuses:
        avg_conf = f"{status['avg_confidence']:.2f}" if status['avg_confidence'] else "N/A"
        print(f"  {status['parsing_status']:<15} - Count: {status['count']:2d}, Avg Confidence: {avg_conf}")
