...
export default function RecommendationsPage() {
  ...
  const fetchRecommendations = useCallback(async () => {
    ...
    try {
      ...
      let url = jobId
        ? `/recruiter/jobs/${jobId}/recommended-candidates?filter_type=${modesStr}`
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(255,138,0,0.08),_transparent_34%),linear-gradient(180deg,#FFFCF8_0%,#FFFFFF_100%)] text-slate-900">
      ...
      `}</style>
      <main className="mx-auto flex h-screen max-w-[1600px] flex-col gap-0 overflow-hidden px-4 py-4">
        {isLocked ? (
          <LockedView featureName="Recommended Talent" />
        ) : (
          <>
            <section
              className="grid min-h-0 grid-cols-1 gap-4 sm:grid-cols-[28%_72%]"
            ...
            >
              <aside className="flex h-full min-h-0 flex-col overflow-hidden rounded-[28px] border border-orange-100/80 bg-white/90 shadow-[0_8px_28px_rgba(255,138,0,0.07)]">
                <div className="flex-shrink-0 border-b border-orange-100/70 bg-gradient-to-r from-[#FFF6ED] to-white px-4 py-2">
                ...
                <div className="flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto p-3">
                  <div className="relative">
                    ...
                    <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3 h-3 text-slate-400 mt-3" />
                    <input
                      type="text"
                      placeholder="Name, role, skills"
                      value={searchTerm}
                      onChange={(e) => setSearchTerm(e.target.value)}
                      className="w-full rounded-2xl border border-slate-200 bg-white py-1.5 pl-8 pr-3 text-xs text-slate-900 outline-none transition-all placeholder:text-slate-400 focus:border-[#FF8A00]/60 focus:ring-2 focus:ring-[#FF8A00]/15"
                    />
                  </div>

                  <div>
                    <label className="mb-1 block text-xs font-bold text-slate-600">Focus</label>
                    <select
                      value={activeFilters[0] || "understanding_and_assessment"}
                      onChange={(e) => toggleFilter(e.target.value)}
                      className="w-full cursor-pointer rounded-2xl border border-slate-200 bg-white px-2 py-1.5 text-xs font-medium text-slate-900 outline-none transition-all focus:border-[#FF8A00]/60 focus:ring-2 focus:ring-[#FF8A00]/15"
                    >
                      <option value="understanding_and_assessment">Fit First</option>
                      <option value="skill_match">Skills First</option>
                    </select>
                  </div>

                    <div>
                      <label className="mb-1 block text-xs font-bold text-slate-600">Experience</label>
                      <select
                        value={filterExperience}
                        onChange={(e) => toggleExperienceFilter(e.target.value)}
                        className="w-full cursor-pointer rounded-2xl border border-slate-200 bg-white px-2 py-1 text-xs font-medium text-slate-900 outline-none transition-all focus:border-[#FF8A00]/60 focus:ring-2 focus:ring-[#FF8A00]/15"
                      >
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full rounded-2xl border border-slate-200 bg-white py-1 pl-8 pr-2 text-xs text-slate-900 outline-none transition-all placeholder:text-slate-400 focus:border-[#FF8A00]/60 focus:ring-2 focus:ring-[#FF8A00]/15"
                        />
                      </div>
                    </div>

                    <div>
                      <label className="mb-1 block text-xs font-bold text-slate-600">Budget</label>
                      <div className="relative">
                    ...
                    <div className="col-span-2">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {[
                          ...
                          { id: "short_notice", label: "Short Notice (â‰¤30d)" },
                          { id: "long_notice", label: "Long Notice (â‰¥60d)" },
                        ].map((f) => (
                          <button
                            key={f.id}
                            onClick={() => setFilterCareerReadiness(filterCareerReadiness === f.id ? "" : f.id)}
                            className={`rounded-full px-2 py-1 text-xs font-medium transition-all ${
                              filterCareerReadiness === f.id
                    ...
                    <div className="col-span-2">
                      <label className="mb-1.5 block text-xs font-bold text-slate-600">Job Search Status</label>
                      <div className="flex flex-wrap gap-1.5">
                        {[
                          { id: "active_job_seeker", label: "Active Job Seeker" },
                          { id: "passive_candidate", label: "Passive Candidate" },
                        ].map((f) => (
                          <button
                            key={f.id}
                            onClick={() => setFilterCareerReadiness(filterCareerReadiness === f.id ? "" : f.id)}
                    ...
                    <div className="col-span-2">
                      <label className="mb-1.5 block text-xs font-bold text-slate-600">Employment Status</label>
                      <div className="flex flex-wrap gap-1.5">
                        {[
                          { id: "between_roles", label: "Between Roles" },
                          { id: "laid_off_recently", label: "Laid Off Recently" },
                          { id: "recent_graduate_student", label: "Student / Graduate" },
                        ].map((f) => (
                          <button
                            key={f.id}
                            onClick={() => setFilterCareerReadiness(filterCareerReadiness === f.id ? "" : f.id)}
                    ...
                    <div className="col-span-2">
                      <label className="mb-1.5 block text-xs font-bold text-slate-600">Work Arrangement</label>
                      <div className="flex flex-wrap gap-1.5">
                        {[
                          { id: "willing_to_relocate", label: "Willing to Relocate" },
                          { id: "remote_only", label: "Remote Only" },
                          { id: "contract_preferred", label: "Contract-Preferred" },
                    ...
                    <div className="col-span-2">
                      <label className="mb-1.5 block text-xs font-bold text-slate-600">Compensation</label>
                      <div className="flex flex-wrap gap-1.5">
                        {[
                          { id: "salary_seeking_raise", label: "Seeking Raise (â‰¥20%)" },
                          { id: "high_fit_by_compensation", label: "Budget Match" },
                          { id: "needs_salary_clarification", label: "Needs Salary Input" },
                        ].map((f) => (
                          <button
                            key={f.id}
                            onClick={() => setFilterCareerReadiness(filterCareerReadiness === f.id ? "" : f.id)}
                    ...
                    <div className="col-span-2">
                      <label className="mb-1.5 block text-xs font-bold text-slate-600">Visa</label>
                      <div className="flex flex-wrap gap-1.5">
                        {[
                          { id: "requires_visa_sponsorship", label: "Requires Sponsorship" },
                        ].map((f) => (
                          <button
                            key={f.id}
                            onClick={() => setFilterCareerReadiness(filterCareerReadiness === f.id ? "" : f.id)}
                  ...
                  <button
                    ...
                    disabled={isSyncing}
                    className="mt-auto inline-flex items-center justify-center gap-2 rounded-2xl bg-[#FF8A00] px-3 py-2 text-xs font-bold uppercase tracking-widest text-white transition-all hover:bg-[#E67A00] disabled:cursor-not-allowed disabled:opacity-50 active:scale-[0.98]"
                  >
                    {isSyncing ? (
      ...
      {postedJobTitle && (
        <div className="px-5 pb-2">
          <div className="rounded-2xl border border-orange-100 bg-[#FFF8F1] px-3 py-2">
            <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#C96B00]">Recommended for</p>
