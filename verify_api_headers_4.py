...
# Combination 4: Token WITHOUT Zoho-encz in lowercase
print('--- Testing Authorization Header with lowercase Zoho-encz ---')
headers['Authorization'] = f'zoho-encz {token}'
try:
...
# Combination 5: API Key Header specifically for ZeptoMail (not documented but used in some SDKs)
print('\\n--- Testing X-ZOHO-API-TOKEN Header ---')
headers = {
  'X-ZOHO-API-TOKEN': f'Zoho-encz {token}',
  'content-type': 'application/json'
}
try:
    response = requests.post('https://api.zeptomail.in/v1.1/email/template', 
